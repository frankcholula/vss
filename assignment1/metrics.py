...
class ClassBasedEvaluator:
    ...
    def create_class_matrix(self, input_image_class: str, retrieved_image_classes: List) -> pd.DataFrame:
    ...
    def plot_class_matrix(self, confusion_df: pd.DataFrame, input_image_class: str):
    ...
    def calculate_precision_recall(self, input_image_class: str, retrieved_image_classes: List, total_relevant_images:int):
    ...
    def calculate_pr_curve(self, input_image_class: str, retrieved_image_classes: list, total_relevant_images:int):
        precision_values = []
        recall_values = []
        for i in range(1, len(retrieved_image_classes) + 1):
            top_retrieved_classes = retrieved_image_classes[:i]        
            precision, recall = self.calculate_precision_recall(input_image_class, top_retrieved_classes, total_relevant_images)
...
class LabelBasedEvaluator:
    def __init__(self, input_image_labels: List[str], retrieved_image_labels: List[List[str]]):
        self.input_image_labels = input_image_labels
        self.retrieved_image_labels = retrieved_image_labels

    def create_labels_matrix(self, input_image_labels: List[str], retrieved_image_labels: List[List[str]]) -> pd.DataFrame:
